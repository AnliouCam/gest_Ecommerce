...
Route::middleware('auth')->group(function () {
    ...
    Route::middleware('role:vendeur,gerant')->prefix('ventes')->name('ventes.')->group(function () {
        ...
        // Products (read-only)
        Route::resource('products', VendeurProductController::class)->only(['index', 'show']);

        // Sales (POS)
        Route::prefix('sales')->name('sales.')->group(function () {
            Route::get('/', [VendeurSaleController::class, 'index'])->name('index');
            Route::get('/create', [VendeurSaleController::class, 'create'])->name('create');
            Route::post('/', [VendeurSaleController::class, 'store'])->name('store');
            Route::get('/search-products', [VendeurSaleController::class, 'searchProducts'])->name('search-products')->middleware('throttle:60,1');
            Route::post('/quick-create-customer', [VendeurSaleController::class, 'quickCreateCustomer'])->name('quick-create-customer')->middleware('throttle:30,1');
            Route::get('/{sale}', [VendeurSaleController::class, 'show'])->name('show');
        });

        // Invoices (PDF) - Rate limited to prevent DoS on CPU-intensive PDF generation
        Route::prefix('invoices')->name('invoices.')->middleware('throttle:10,1')->group(function () {
