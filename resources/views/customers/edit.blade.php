...
<div class="max-w-xl mx-auto">
    ...
    <div class="mb-6">
        ...
        <h1 class="mt-2 text-2xl font-bold text-gray-900">Modifier le client</h1>
        <p class="mt-1 text-sm text-gray-500">{{ $customer->name }} - {{ $customer->sales_count }} achat(s)</p>
    </div>

    <!-- Form -->
    <form action="{{ route('ventes.customers.update', $customer) }}" method="POST" class="space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 space-y-4">
            ...
            <div>
                ...
                <textarea name="address" id="address" rows="3"
                    ...
                    placeholder="Ex: 123 Rue de la Paix, 75001 Paris">{{ old('address', $customer->address) }}</textarea>
                @error('address')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
